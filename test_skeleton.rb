...
class TestSkeleton
  ...
  def reverse_array(number)
    return number.to_s.split(//).reverse.map{|x| x.to_i}
  end
  ...
  def high_and_low(test_string)
  	numbers = test_string.split(' ').map{|item| item.to_i}.sort()
  ...
  def my_languages(hash)
    dict = hash.select {|key, value| value >= 60 }
	  dict = dict.sort_by{|key, value| value}.reverse().to_h
	  return dict.keys()
  end
  ...
  def middle_chars(test_string)
    n = test_string.length()
    mid = (n/2).to_i - 1
    return n % 2 == 0? test_string[mid..mid+1] : test_string[mid+1]
  end
end
